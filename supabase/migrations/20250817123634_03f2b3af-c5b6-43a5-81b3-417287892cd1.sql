-- Setup email triggers and cron job for complete email flow
...
-- Update the handle_new_user function to properly call send-email function
...
-- Update the send_tournament_created_email function  
...
-- Update the review request function to be more robust
...
BEGIN
  ...
  LOOP
    ...
    -- Skip if no organizer email found
    IF organizer_email IS NULL THEN
      CONTINUE;
    END IF;
...
-- Set up daily cron job to send review request emails at 9 AM UTC
...
-- Add some logging/monitoring functions
CREATE OR REPLACE FUNCTION public.get_email_stats()
RETURNS TABLE(
  total_tournaments integer,
  tournaments_with_review_emails integer,
  pending_review_emails integer
) 
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
  RETURN QUERY
  SELECT 
    (SELECT COUNT(*)::integer FROM public.tournaments) as total_tournaments,
    (SELECT COUNT(*)::integer FROM public.tournament_review_emails_sent) as tournaments_with_review_emails,
    (SELECT COUNT(*)::integer 
     FROM public.tournaments t 
     WHERE t.end_date::date <= (CURRENT_DATE - interval '2 days')::date
       AND NOT EXISTS (
         SELECT 1 FROM public.tournament_review_emails_sent 
         WHERE tournament_id = t.id
       )
    ) as pending_review_emails;
END;
$function$;